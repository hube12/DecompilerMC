...
def decompilefern(decompVersion, version):
    ...
    if path.exists() and fernflower.exists():
        ...
        subprocess.run(['java', '-jar', fernflower.__str__(), "-hes=0 -hdc=0 -dgs=1 -ren=1 -log=ERROR", path.__str__(), f'./src/{decompVersion}'], check=True)
        print(f'- Removing -> {version}-temp.jar')
        os.remove(f'./src/{version}-temp.jar')
        print(f'- Extracting -> {version}.jar')
...
def decompileforge(decompVersion, version):
    print('=== Decompiling using ForgeFlower (not silent) ===')
    t = time.time()
    path = Path(f'./src/{version}-temp.jar')
    forgeflower = Path('./lib/forgeflower-1.5.380.34.jar')
    if path.exists() and forgeflower.exists():
        path = path.resolve()
        forgeflower = forgeflower.resolve()
        subprocess.run(['java', '-jar', forgeflower.__str__(), "-hes=0 -hdc=0 -dgs=1 -ren=1 -log=ERROR", path.__str__(), f'./src/{decompVersion}'], check=True)
        print(f'- Removing -> {version}-temp.jar')
        os.remove(f'./src/{version}-temp.jar')
        with zipfile.ZipFile(f'./src/{decompVersion}/{version}-temp.jar') as z:
            z.extractall(path=f'./src/{decompVersion}')
...
def main():
    ...
    decompiler = input("Please input you decompiler choice: forgeflower (ff) or fernflower (f) or cfr (default: cfr) : ")
    decompiler = decompiler if decompiler.lower() in ["fernflower", "cfr", "f", "ff", "forgeflower"] else "cfr"
    ...
    if r == 'y':
        if decompiler.lower() == "cfr":
            ...
            if decompiler.lower() == "ff" or decompiler.lower() == "forgeflower":
                decompileforge(decompVersion, version)
            else:
